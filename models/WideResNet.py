...
class wide_basic(nn.Module):
    ...
    def forward(self, x):
        out = self.dropout(self.conv1(self.JumpReLU(self.bn1(x))))
        out = self.conv2(self.JumpReLU(self.bn2(out)))
        out += self.shortcut(x)

        return out

class WideResNet(nn.Module):
    def __init__(self, depth, widen_factor, dropout_rate, num_classes, level=1, jump=0.0):
